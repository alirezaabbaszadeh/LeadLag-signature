...
try:  # pragma: no cover - optional dependency in the execution environment
    ...
    except ImportError:  # pragma: no cover - allow the module to load without gym
...
class LeadLagEnv(gym.Env):
    ...
    def __init__(
        ...
        *,
        ...
        include_strength_feature:
            ...
            observation vector.
        cache_returns:
            If ``True`` and no universe filtering is provided, pre-compute log
            returns once and reuse them across steps.  This significantly
            reduces the per-step computational cost when the universe is fixed.
        """

        if gym is None or spaces is None:
            raise ImportError(
                "LeadLagEnv requires `gymnasium` (>=0.28) or `gym`. Install it via "
        ...
        self.cache_returns = bool(cache_returns and universe is None)

        self._price_for_returns = self.price_data.ffill()
        if self.cache_returns:
            # Log returns are required for the lead-lag computation.  We pre-
            # compute them once so each ``step`` only has to slice the window.
            self._cached_returns = (
                np.log(self._price_for_returns).diff().fillna(0.0)
            )
        else:
    ...
    def _compute_leadlag_matrix(self, index: int, lookback: int) -> pd.DataFrame:
        ...
        if self._cached_returns is not None:
            log_returns = self._cached_returns.iloc[start_idx : index + 1]
        else:
            ...
            log_returns = self.analyzer._compute_log_returns_for_window(
                self._price_for_returns, start_ts, end_ts
            )
